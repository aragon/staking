...
contract('Staking app, Time locking', accounts => {
  ...
  const approveStakeAndLock = async(data, lockAmount = DEFAULT_AMOUNT / 2, stakeAmount = DEFAULT_AMOUNT) => {
    ...
    const r = await staking.lock(lockAmount, managerAddress, data)
  ...
  beforeEach(async () => {
    ...
    const managerContract = await TimeLockManagerMock.new()
    manager = managerContract
    managerAddress = managerContract.address
  })
  ...
  it('changes lock data', async () => {
    ...
    await manager.setLockData(stakingAddress, owner, lockId, data2)
