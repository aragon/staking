  ...
  context('Transfers', async () => {
    ...
    const transfersFromLock = (transferType) => {
      it('transfers', async () => {
        ...
        await lockManager[transferType](staking.address, owner, user1, ZERO_ADDRESS, transferAmount)

        assert.equal((await staking.unlockedBalanceOf(owner)).toString(), DEFAULT_STAKE_AMOUNT - DEFAULT_LOCK_AMOUNT, "Owner balance should match")
        const userUnlockedBalance = transferType == 'transferFromLock' ? transferAmount : 0
        assert.equal((await staking.unlockedBalanceOf(user1)).toString(), userUnlockedBalance, "User 1 unlocked balance should match")

        const userExternalBalance = transferType == 'transferFromLock' ? 0 : transferAmount
        assert.equal((await token.balanceOf(user1)).toString(), userExternalBalance, "User 1 external balance should match")

        // total stake
        const totalStaked = transferType == 'transferFromLock' ? DEFAULT_STAKE_AMOUNT : DEFAULT_STAKE_AMOUNT - transferAmount
      ...
      it('transfers the whole lock amount', async () => {
        await approveStakeAndLock(lockManager.address)
        await lockManager[transferType](staking.address, owner, user1, ZERO_ADDRESS, DEFAULT_LOCK_AMOUNT)

        assert.equal((await staking.unlockedBalanceOf(owner)).toString(), DEFAULT_STAKE_AMOUNT - DEFAULT_LOCK_AMOUNT, "Owner balance should match")
        const userUnlockedBalance = transferType == 'transferFromLock' ? DEFAULT_LOCK_AMOUNT : 0
        assert.equal((await staking.unlockedBalanceOf(user1)).toString(), userUnlockedBalance, "User 1 unlocked balance should match")

        const userExternalBalance = transferType == 'transferFromLock' ? 0 : DEFAULT_LOCK_AMOUNT
        assert.equal((await token.balanceOf(user1)).toString(), userExternalBalance, "User 1 external balance should match")

        // total stake
        const totalStaked = transferType == 'transferFromLock' ? DEFAULT_STAKE_AMOUNT : DEFAULT_STAKE_AMOUNT - DEFAULT_LOCK_AMOUNT
        assert.equal((await staking.totalStaked()).toString(), totalStaked, "Total stake should match")
      ...
      it('fails transferring zero tokens', async () => {
        await approveStakeAndLock(lockManager.address)
        await assertRevert(lockManager[transferType](staking.address, owner, user1, ZERO_ADDRESS, 0), STAKING_ERRORS.ERROR_AMOUNT_ZERO)
      })

      it('fails transferring more than locked balance', async () => {
        await approveStakeAndLock(lockManager.address)
        await assertRevert(lockManager[transferType](staking.address, owner, user1, ZERO_ADDRESS, DEFAULT_LOCK_AMOUNT + 1), STAKING_ERRORS.ERROR_NOT_ENOUGH_LOCK)
      })

      it('fails if sender is not manager', async () => {
        await approveStakeAndLock(user1)
        await assertRevert(lockManager[transferType](staking.address, owner, user1, ZERO_ADDRESS, DEFAULT_LOCK_AMOUNT), STAKING_ERRORS.ERROR_NOT_ENOUGH_LOCK)
      })

      it('fails transferring from unlocked lock', async () => {
        await approveStakeAndLock(lockManager.address)
        // unlock
        await lockManager.decreaseAndRemoveManager(staking.address, owner)
        await assertRevert(lockManager[transferType](staking.address, owner, user2, ZERO_ADDRESS, DEFAULT_LOCK_AMOUNT, { from: user1 }), STAKING_ERRORS.ERROR_NOT_ENOUGH_LOCK)
      })
    }

    context('From Lock', async () => {
      context('within Staking app', () => {
        transfersFromLock('transferFromLock')
      ...
      context('to external balance (unstaked)', () => {
        transfersFromLock('transferFromLockAndUnstake')
      })
    })
  })
})
